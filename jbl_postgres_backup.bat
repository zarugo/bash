...
:reset_location:
SET BACKUP_LOCATION=C:\Backup_JMS\jbl
IF NOT EXIST %BACKUP_LOCATION% (
				mkdir %BACKUP_LOCATION%)	
GOTO backup
...
SET BACKUP_FILE=jbl_%DATESTR%_%TIMESTR%.backup
SET LOGFILE=%BACKUP_LOCATION%\logs\jbl_postgres_backup_%DATESTR%_%TIMESTR%.log
