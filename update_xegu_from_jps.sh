...
if [[ -f /home/root/JPSApps/JPSApplication/Resources/xegu.zip ]]; then
  unzip -o /home/root/JPSApps/JPSApplication/Resources/xegu.zip -d /home/root/JPSApps/JPSApplication/Resources &>/dev/null
  rm -f /home/root/JPSApps/JPSApplication/Resources/xegu.zip
fi
cp -r /home/root/JPSApps/JPSApplication/Resources/xegu/ /mnt/sd/ 2>/dev/null
if [[ $? -eq 0 ]]; then
  moved=true
  rm -fr /home/root/JPSApps/JPSApplication/Resources/xegu 2>/dev/null
else
  moved=false
fi
...
function update_binary () {
 ...
 if [[ $ul ]]; then
  ...
  if [[ $? -eq 0 ]]; then
    update_bin_done=true
  else
    update_bin_done=false
  fi
...
#output
printf '{\n  "display":"%s",\n  "moved_to_sd":%s,\n  "lang_needs_update":%s,\n  "bin_needs_update":%s,\n  "update_bin_done":%s,\n  "update_lang_done":%s\n}\n' "$type" "$moved" "$langupdate" "$binupdate" "$update_bin_done" "$update_lang_done"
