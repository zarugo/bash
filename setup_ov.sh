...
OV_Config   "Manually edit OV json configuration file"
...
while [ -z $result ] || [ $result == "1" ];
     do
        ...
        while [[ -z $ip_result ]] || [[ "$ip_result" == "1" ]] ; do
            ipaddr=$(whiptail --backtitle "OV Network Setup" --nocancel --inputbox "IP Address" $HEIGHT $WIDTH  3>&1 1>&2 2>&3)
            if ! [[ $ipaddr =~ $ip4 ]]; then
        ...
        while [[ -z $sm_result ]] || [[ "$sm_result" == "1" ]] ; do
            netmask=$(whiptail --backtitle "OV Network Setup" --nocancel --inputbox "Subnet Mask" $HEIGHT $WIDTH  3>&1 1>&2 2>&3)
            if ! [[ $netmask =~ $ip4 ]]; then
        ...
        while [[ -z $gw_result ]] || [[ "$gw_result" == "1" ]] ; do
            gateway=$(whiptail --backtitle "OV Network Setup" --nocancel --inputbox "Default Gateway" $HEIGHT $WIDTH  3>&1 1>&2 2>&3)
            if ! [[ $gateway =~ $ip4 ]]; then
        ...
        while [[ -z $dns_result ]] || [[ "$dns_result" == "1" ]] ; do
            dns1=$(whiptail --backtitle "OV Network Setup" --nocancel --inputbox "Primary DNS" $HEIGHT $WIDTH  3>&1 1>&2 2>&3)
            if ! [[ $dns1 =~ $ip4 ]]; then
        ...
        whiptail --backtitle "OV Network Setup" --title "Are the settings correct?" --yesno "\n IP Adress: $ipaddr \n Netmask: $netmask \n Gateway: $gateway \n DNS: $dns1 \n" $HEIGHT $WIDTH 3>&1 1>&2 2>&3
	result=$?
#reset the loops in case we want to go back and assign a different network config in the same session
	...
	if ! [[ $? = "0" ]]
	then
		whiptail --backtitle "OV Network Setup" --title "ERROR!" --msgbox "Settings are NOT written to /etc/network/interfaces and/etc/resolv.conf, be sure that you are root!" $HEIGHT $WIDTH
	else
		whiptail --backtitle "OV Network Setup" --title "OK!" --msgbox "New settings are written to config file, please reboot to apply" $HEIGHT $WIDTH
...
case $CHOICE in
	...
	;;
	OV_Config)
	     nano /home/pi/JPSApps/JPSApplication/Resources/www/webcfgtool/apsapp/ApsApp/ConfigData.json
	;;
    Reboot)
